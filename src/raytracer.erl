...
p3()->
    ...
    Lower_left_corner = numerl:sub(
                            numerl:sub(
                                numerl:sub(
                                    Origin,
                                    numerl:divide(Horizontal,2)),
                                numerl:divide(Vertical,2)),
                            vec3(0, 0, Focal_length)),
    %io:format("Lower left corner is at ~p ~n", [numerl:print(Lower_left_corner)]),

    GenPixel = fun(I,J)->
                    U = I/(Image_width-1),
                    V = J/(Image_height-1),
                    %lower_left_corner + u*horizontal + v*vertical - origin
                    R = #ray{orig=Origin, dir=numerl:sub(
                                                numerl:add(
                                                    numerl:add(
                                                        Lower_left_corner,
                                                        numerl:mult(Horizontal,U)),
                                                    numerl:mult(Vertical, V)),
                                                Origin)},
                    Color = ray_color(R),
